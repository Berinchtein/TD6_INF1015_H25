...
class Board; class Tile; class Piece; class Rook; class King; class Knight; // Permet d'utiliser les types alors qu'ils seront défini après.
...
public:
    ...
    std::unique_ptr<Tile> createTile(const std::string& tileName, bool isOccupied) const;
    std::unique_ptr<Piece> createPiece(char pieceName) const;
